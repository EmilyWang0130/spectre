...
#include "Evolution/Systems/GrMhd/ValenciaDivClean/FiniteDifference/Flat.hpp"
...
#include "NumericalAlgorithms/FiniteDifference/Flat.hpp"
...
FlatPrim::FlatPrim(const bool reconstruct_rho_times_temperature)
    : reconstruct_rho_times_temperature_(reconstruct_rho_times_temperature) {}

FlatPrim::FlatPrim(CkMigrateMessage* const msg) : Reconstructor(msg) {}

std::unique_ptr<Reconstructor> FlatPrim::get_clone() const {
  return std::make_unique<FlatPrim>(*this);
}

void FlatPrim::pup(PUP::er& p) {
  Reconstructor::pup(p);
  p | reconstruct_rho_times_temperature_;
}

// NOLINTNEXTLINE
PUP::able::PUP_ID FlatPrim::my_PUP_ID = 0;

template <size_t ThermodynamicDim>
void FlatPrim::reconstruct(
  ...
  reconstruct_prims_work<prims_to_reconstruct_tags>(
      ...
      [](auto upper_face_vars_ptr, auto lower_face_vars_ptr,
        ...
        ::fd::reconstruction::flat(upper_face_vars_ptr, lower_face_vars_ptr,
...
template <size_t ThermodynamicDim>
void FlatPrim::reconstruct_fd_neighbor(
  ...
  reconstruct_fd_neighbor_work<prims_to_reconstruct_tags,
      ...
      [](const auto tensor_component_on_face_ptr,
        ...
        ::fd::reconstruction::reconstruct_neighbor<
            Side::Lower, ::fd::reconstruction::detail::FlatReconstructor>(
      ...
      [](const auto tensor_component_on_face_ptr,
         ...
         const Direction<dim>& local_direction_to_reconstruct) {
        ::fd::reconstruction::reconstruct_neighbor<
            Side::Upper, ::fd::reconstruction::detail::FlatReconstructor>(
            tensor_component_on_face_ptr, tensor_component_volume,
            tensor_component_neighbor, subcell_extents, ghost_data_extents,
            local_direction_to_reconstruct);
      },
...
bool FlatPrim::reconstruct_rho_times_temperature() const {
...
#define INSTANTIATION(r, data)                                              \
  template void FlatPrim::reconstruct(                                      \
  ...
  template void FlatPrim::reconstruct_fd_neighbor(                          \
