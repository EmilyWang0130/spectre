...
#include "NumericalAlgorithms/FiniteDifference/FirstOrder.hpp"
...
FirstOrderPrim::FirstOrderPrim(const bool reconstruct_rho_times_temperature)
    : reconstruct_rho_times_temperature_(reconstruct_rho_times_temperature) {}

FirstOrderPrim::FirstOrderPrim(CkMigrateMessage* const msg)
    : Reconstructor(msg) {}

std::unique_ptr<Reconstructor> FirstOrderPrim::get_clone() const {
  return std::make_unique<FirstOrderPrim>(*this);
}

void FirstOrderPrim::pup(PUP::er& p) {
  Reconstructor::pup(p);
  p | reconstruct_rho_times_temperature_;
}

// NOLINTNEXTLINE
PUP::able::PUP_ID FirstOrderPrim::my_PUP_ID = 0;

template <size_t ThermodynamicDim>
void FirstOrderPrim::reconstruct(
...
template <size_t ThermodynamicDim>
void FirstOrderPrim::reconstruct_fd_neighbor(
...
bool FirstOrderPrim::reconstruct_rho_times_temperature() const {
...
#define INSTANTIATION(r, data)                                              \
  template void FirstOrderPrim::reconstruct(                                \
  ...
  template void FirstOrderPrim::reconstruct_fd_neighbor(                    \
