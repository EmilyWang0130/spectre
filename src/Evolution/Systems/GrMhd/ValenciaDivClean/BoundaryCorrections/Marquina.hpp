...
class Marquina final : public evolution::BoundaryCorrection {
 public:
  /// Package-data tags shared with `MarquinaCpm`. Public so `MarquinaCpm`
  /// can reuse the tag types verbatim without inheritance (SpECTRE
  /// requires all createable BoundaryCorrections to be `final`).
  ...
  /// Wrapper — calls `dg_package_data_impl` with the CPM flag off. The
  /// full 6-mode characteristic decomposition is used for the flux
  /// packaging.
  ...
  /// Wrapper — calls `dg_boundary_terms_impl` with the CPM flag off.
  ...
  /// Shared implementation used by both `Marquina` and `MarquinaCpm`.
  /// When `use_cpm_degenerate_block` is `true`, only the two acoustic
  /// eigenvectors are computed and the degenerate-block flux is
  /// assembled via the spectral projector complement (see Fedkiw,
  /// Merriman & Osher 1997 §4.3, eqs 6 and 47). Left public so that
  /// `MarquinaCpm` can dispatch to it directly (SpECTRE requires all
  /// createable BoundaryCorrections to be `final`, ruling out
  /// inheritance).
  static double dg_package_data_impl(
      bool use_cpm_degenerate_block,
      gsl::not_null<Scalar<DataVector>*> packaged_tilde_d,
      gsl::not_null<Scalar<DataVector>*> packaged_tilde_ye,
      gsl::not_null<Scalar<DataVector>*> packaged_tilde_tau,
      gsl::not_null<tnsr::i<DataVector, 3, Frame::Inertial>*> packaged_tilde_s,
  ...
  static void dg_boundary_terms_impl(
      bool use_cpm_degenerate_block,
      gsl::not_null<Scalar<DataVector>*> boundary_correction_tilde_d,
      gsl::not_null<Scalar<DataVector>*> boundary_correction_tilde_ye,
      gsl::not_null<Scalar<DataVector>*> boundary_correction_tilde_tau,
