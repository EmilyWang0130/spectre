...
enum class MarquinaCharacteristicsMethod {
  ...
  /// pays off when they collapse.  (The fast and GLM-scalar waves stay
  /// analytic.)
...
class Marquina final : public evolution::BoundaryCorrection {
 ...
 public:
  ...
  struct DegeneracyTolerance {
    ...
    static constexpr Options::String help = {
        "Speed-gap tolerance (on the c_h = 1 speed scale): an Mhd wave is "
        "treated "
        "as degenerate -- and reconstructed by the complementary projection "
        "instead of the analytic per-wave decomposition -- when its "
        "characteristic "
        "speed lies within this tolerance of another wave's speed (only used "
        "with "
        "AnalyticWithComplementaryProjection). The speed gap tracks the "
        "degeneracy "
...
/// \cond
template <>
struct Options::create_from_yaml<grmhd::ValenciaDivClean::BoundaryCorrections::
