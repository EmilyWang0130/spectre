...
// ideal-gas EOS helpers (Gamma threaded explicitly -- no mutable global state
// so this is safe to call concurrently).  Match ref_hlld IdealEOS.
...
struct RecState {
  ...
  std::array<double, 2> compute_max_characteristicsSR(
    ...
    {
      const double v_dot_bb_perp =
          (b[0] - P[WVX + dir] * P[BX + dir]) * lorentzi;
      ...
      if (mask_a1)
        lambda_plus_no_bbx = -2.0 * a0 / (a1 + s);
      if (mask_am)
        lambda_minus_no_bbx = -2.0 * a0 / (a1 - s);
    }
    ...
    double lambda_plus, lambda_minus;
    {
      ...
      double y1, y2, y3, y4;
      {
        ...
        const double r =
            (2.0 * c2 * c2 * c2 - 9.0 * c1 * c2 + 27.0 * c0) / 54.0;
  ...
  explicit RecState(const std::array<double, 9>& P, double gamma_)
      : gamma(gamma_) {
    U[BBX] = P[BX];
    U[BBY] = P[BY];
    U[BBZ] = P[BZ];
...
struct CDState {
  ...
  double update(RotState<dir>& LL, RotState<dir>& RR, double ptot) {
    ...
    UL[BY] = RR.U[BY] * (RR.K[VX] - RR.v[VX]) -
             LL.U[BY] * (LL.K[VX] - LL.v[VX]) +
             RR.U[BX] * (RR.v[VY] - LL.v[VY]);
    UL[BZ] = RR.U[BZ] * (RR.K[VX] - RR.v[VX]) -
             LL.U[BZ] * (LL.K[VX] - LL.v[VX]) +
    ...
    auto compute_v = [&](std::array<double, 3>& vv, RotState<dir>& SS,
      ...
      const double K2 =
          SS.K[0] * SS.K[0] + SS.K[1] * SS.K[1] + SS.K[2] * SS.K[2];
      const double mK2 = 1. - K2;
      const double KdotB =
          SS.K[0] * UU[BBX] + SS.K[1] * UU[BBY] + SS.K[2] * UU[BBZ];
  ...
  void compute_cons(RotState<dir>& LL, RotState<dir>& RR, double ptot) {
    ...
    auto cmp_cons = [&](RotState<dir>& SS, std::array<double, NUM>& UU) {
      ...
      UU[UE] =
          (SS.lambda * SS.U[UE] - SS.U[SX] + ptot * lambda - vdotB * UL[BX]) *
          lvi;
...
struct HLLState {
  ...
  void compute_ptot() {
    ...
    const double Pmin = std::max(
        Uin[DENS] * (std::cbrt(27. / 4. * dn) - 1. - 0.5 * B2) - Uin[UE],
        1.e-100);
    ...
    if (!fail_mask) {
      for (int nn = 0; nn < num_it; ++nn) {
        ...
        const double E1 =
            1. / 3. * an -
            2. / 3. * an * std::cos(2. / 3. * phi + 2. / 3. * M_PI);
        hW = E1 - B2;
        hWi = 1. / hW;
        ...
        const bool mask_conv = std::fabs(press - press_prev) <
                               c2p_tol * std::max(press_prev, press);
        if (mask_conv)
          break;
        const double Rr = (press - press_prev) / (press_prev - press_pp);
        double Paitken =
            std::fabs(press_prev + (press - press_prev) / (1. - Rr));
