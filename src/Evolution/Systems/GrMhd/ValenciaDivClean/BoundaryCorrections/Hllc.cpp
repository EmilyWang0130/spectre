...
Hllc::Hllc(const double magnetic_field_magnitude_for_hydro,
...
Hllc::Hllc(CkMigrateMessage* /*unused*/) {}

std::unique_ptr<evolution::BoundaryCorrection> Hllc::get_clone() const {
  return std::make_unique<Hllc>(*this);
}

void Hllc::pup(PUP::er& p) {
  BoundaryCorrection::pup(p);
  p | magnetic_field_magnitude_for_hydro_;
  p | light_speed_density_cutoff_;
}

double Hllc::dg_package_data(
...
void Hllc::dg_boundary_terms(
  ...
  // flip. See the notes in Hllc.hpp for the derivation.
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (lambda_L >= 0.0) {
      ...
      if (bad_rat_branch or bad_dir_branch or bad_input) {
        ERROR("[Hllc DIAG] Degenerate quadratic at grid point i="
              << i << "\n  a=" << a << " b=" << b << " c=" << c
              << " disc=" << discriminant << "\n  lambda_L=" << lambda_L
              << " lambda_R=" << lambda_R << "\n  L: D=" << D_L
...
bool operator!=(const Hllc& lhs, const Hllc& rhs) { return not(lhs == rhs); }

// NOLINTNEXTLINE
PUP::able::PUP_ID Hllc::my_PUP_ID = 0;
}  // namespace grmhd::ValenciaDivClean::BoundaryCorrections
