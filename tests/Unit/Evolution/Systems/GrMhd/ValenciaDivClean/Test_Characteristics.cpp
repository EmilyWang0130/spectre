...
// documented zeta = 0 basis (R4[5] = 1, L4[5] = 1, L4[0] = -Y_e -- the trivial
// passive-advection form that the test above verifies the PHYSICAL zeta does
// NOT produce). The acoustic pair R+/R- is zeta-invariant and must be
// unchanged; the composition mode R4 and the left vectors must move.
//
// Note we deliberately do NOT re-check the eigensystem relation A.R = lambda R
// for the zeta = 0 basis: the flux Jacobian A is still built from the physical
// EoS, so that relation is expected to fail. The switch produces a controlled
// comparison basis, not an eigenbasis of the physical system.
void test_use_physical_zeta_switch() {
  const size_t num_points = 3;
  const DataVector used_for_size(num_points, 0.0);
  ...
  const auto zeta = equation_of_state.zeta_from_density_and_temperature(
      rest_mass_density, temperature, electron_fraction);
  // The switch is only meaningful if the table really supplies zeta != 0.
  REQUIRE(max(abs(get(zeta))) > 1.0e-12);
  ...
  constexpr size_t matrix_size = 6;
  const auto eigenvectors = [&](const bool use_physical_zeta) {
    tnsr::ij<DataVector, matrix_size> modes{num_points};
    tnsr::IJ<DataVector, matrix_size> projectors{num_points};
    grmhd::ValenciaDivClean::characteristic_eigenvectors_hydro(
        make_not_null(&modes), make_not_null(&projectors), spatial_velocity,
        rest_mass_density, specific_internal_energy, specific_enthalpy,
        electron_fraction, lorentz_factor, unit_normal, spatial_metric,
        equation_of_state, use_physical_zeta);
    return std::pair{modes, projectors};
  };

  const auto [modes_phys, proj_phys] = eigenvectors(true);
  const auto [modes_zero, proj_zero] = eigenvectors(false);

  namespace VDC = grmhd::ValenciaDivClean;
  constexpr double tol = 1.0e-12;

  // 1. The acoustic pair is zeta-invariant.
  for (const size_t wave : {static_cast<size_t>(VDC::HydroVectorR::Rplus),
                            static_cast<size_t>(VDC::HydroVectorR::Rminus)}) {
    for (size_t comp = 0; comp < matrix_size; ++comp) {
      CHECK_ITERABLE_APPROX(modes_phys.get(wave, comp),
                            modes_zero.get(wave, comp));
    }
  }

  // 2. Switching the coupling off must reproduce the documented zeta = 0
  //    basis for the composition mode. These are exactly the trivial values
  //    that test_tabulated3d_kappa_and_zeta_in_characteristics() checks the
  //    physical zeta does NOT produce.
  CHECK(max(abs(modes_zero.get(VDC::HydroVectorR::R4, 5) - 1.0)) < tol);
  CHECK(max(abs(proj_zero.get(VDC::HydroVectorL::L4, 5) - 1.0)) < tol);
  CHECK(max(abs(proj_zero.get(VDC::HydroVectorL::L4, 0) +
                get(electron_fraction))) < tol);

  // 3. ... and the physical result must genuinely differ, so the switch is
  //    doing observable work rather than being a no-op at this state.
  CHECK(max(abs(modes_phys.get(VDC::HydroVectorR::R4, 5) -
                modes_zero.get(VDC::HydroVectorR::R4, 5))) > 1.0e-12);
  CHECK(max(abs(proj_phys.get(VDC::HydroVectorL::L4, 5) -
                proj_zero.get(VDC::HydroVectorL::L4, 5))) > 1.0e-12);

  // 4. The switch must be pure: calling again with `true` reproduces the
  //    original bit-for-bit (no state leaked into the EoS or temporaries).
  const auto [modes_again, proj_again] = eigenvectors(true);
  for (size_t wave = 0; wave < matrix_size; ++wave) {
    for (size_t comp = 0; comp < matrix_size; ++comp) {
      CHECK_ITERABLE_APPROX(modes_phys.get(wave, comp),
                            modes_again.get(wave, comp));
      CHECK_ITERABLE_APPROX(proj_phys.get(wave, comp),
                            proj_again.get(wave, comp));
    }
  }
}
  ...
  for (const auto& direction : Direction<3>::all_directions()) {
    ...
    for (size_t point = 0; point < num_points; ++point) {
      ...
      quad_ref::characteristic_eigenvectors_mhd(
          &q_right, &q_left, q_speeds, q_v, q_B, get(rest_mass_density)[point],
          get(specific_internal_energy)[point], get(lorentz_factor)[point],
          get(specific_enthalpy)[point], q_g, q_n, q_t1, q_t2);
      ...
      // basis becomes ill-conditioned.  It is left as a diagnostic rather than
      // a hard CHECK because a random draw can land close to a degeneracy; see
      // the biorthogonality study in runs-ai/mhd_eigenvectors/.
    ...
    for (size_t point = 0; point < num_points; ++point) {
      for (size_t i = 0; i < 9; ++i) {
        ...
        for (size_t m = 0; m < 9; ++m) {
          ...
          rnorm =
              std::max(rnorm, std::abs(right_eigenvectors.get(i, m)[point]));
...
void test_mhd_characteristics_errors(const bool output) {
  ...
  const std::array<Config, 6> configs{{
      ...
      // stays tiny while the boosted matrix grows ill-conditioned -- this is
      // the
      ...
      // degeneracy LIFTS under boost (the gap grows to ~4e-3 by W~2), so the
      // gap
      // is never simultaneously tiny and at high W, and the numeric solver
      // stays
  ...
  for (const auto& config : configs) {
    ...
    for (size_t iw = 0; iw < n_w; ++iw) {
      ...
      std::array<std::array<quad_ref::Quad, 9>, 9> q_left{};
      quad_ref::characteristic_eigenvectors_mhd(
          &q_right, &q_left, q_speeds, q_v, q_B, get(rest_mass_density)[0],
      ...
      for (size_t wi = 0; wi < 9; ++wi) {
        for (size_t wj = wi + 1; wj < 9; ++wj) {
          speed_gap =
              std::min(speed_gap, std::abs(static_cast<double>(q_speeds[wi]) -
                                           static_cast<double>(q_speeds[wj])));
        }
      }

      // EIGENPROBLEM CHECK: do the analytic speeds & eigenvectors actually
      // solve A.R = y R and L.A = y L for the conserved characteristic matrix A
      // = "As" (flux_jacobian_mhd)?  This is independent of biorthogonality and
      // of any left eigenvector, and is the authoritative test that the
      // eigenvectors are correct.  Residual normalized per wave by |R| (resp.
      // |L|).
      ...
      // eig_resid_iso measures only the conditioning of the eigenvector
      // FORMULAS (their 1/gap factors blow up near a degeneracy).  Comparing
      // eig_resid (full) with eig_resid_iso disentangles speed-solver error
      // (grows with W) from eigenvector-formula error (near degeneracies).
      ...
      // speeds (modes_1w / projectors_1w), with the 1/W speed as the
      // eigenvalue.
      ...
      for (size_t w = 0; w < 9; ++w) {
        ...
        for (size_t m = 0; m < 9; ++m) {
          ...
          la_resid = std::max(la_resid,
                              std::abs(la - y_1w * projectors_1w.get(w, m)[0]));
      ...
      // general per-point eigensolver (numerical_characteristics /
      // blaze::geev). Unlike the analytic closed form, geev returns
      // eigenvectors for its own computed eigenvalues, so its eigenproblem
      // residual stays near machine precision wherever it succeeds -- which
      // motivates it as a fallback near a degeneracy.  +inf marks a genuine
      // failure (caught here -- SpECTRE ERROR/ASSERT throw): geev cannot
      // resolve two real eigenvalues closer than ~eps*cond(R), so where the
      // speed_gap underflows that limit (the near_type_I config at high W: the
      // gap stays tiny while cond(R) grows with the boost) it returns a
      // spurious complex eigenvalue and trips the real-spectrum check.  We
      // record that as the solver's limit rather than aborting.  (near_type_II
      // lifts under boost, so its gap stays resolvable and geev never fails.)
      ...
      // biorthogonality comparison below; num_ok records whether geev
      // succeeded.
      ...
      if (output and (iw == 0 or iw == 6 or iw == n_w / 2 or iw == n_w - 1)) {
        for (size_t wi = 0; wi < 9; ++wi) {
          for (size_t wj = 0; wj < 9; ++wj) {
            double lr = 0.0;
            for (size_t n = 0; n < 9; ++n) {
      ...
      for (size_t wave = 0; wave < 9; ++wave) {
        ...
        for (size_t n = 0; n < 9; ++n) {
          r_max =
              std::max(r_max, std::abs(modes.get(wave, n)[0] -
                                       static_cast<double>(q_right[wave][n])));
          l_max =
              std::max(l_max, std::abs(projectors.get(wave, n)[0] -
      ...
      // W / extreme magnetization, where even double and quad diverge by
      // O(100). Recorded to mhd_eigenvector_errors.tsv and plotted in
      ...
      const double biorth_norm_1w = normalized_offdiag(projectors_1w, modes_1w);
      ...
      if (output) {
        ...
        out_prec << config.name << '\t' << W << '\t' << speed_gap << '\t'
                 ...
                 << analytic_speed_err << '\t' << speed_err_1w << '\t'
                 << max_RL_err_1w << '\t' << biorth_norm_1w << '\t'
                 << eig_resid_1w << '\t' << biorth_norm_whole << '\t'
                 << biorth_norm_iso << '\t' << eig_resid << '\t'
                 << eig_resid_right << '\t' << eig_resid_left << '\t'
                 << num_eig_resid << '\t' << num_speed_err << '\t'
                 << eig_resid_iso << '\t' << num_biorth << '\n';
      }
    ...
    // (where the eigenvector basis becomes ill-conditioned) and at high W
    // (where the magnetosonic speed solver loses precision).  Because this
    // sweep deliberately probes those regimes, the error is logged (to
...
// QUAD-precision analytic reconstruction sum_i lam_i^q R_i^q (L_i^q)^T
// (accurate to gaps ~1e-30, unlike double ~1e-16).  We compare:
//   * all-analytic:   sum over all 9 double modes (biorthonormality is lost
//   near
...
void test_degeneracy_parameter_sweep(const bool output) {
  const ScopedFpeState disable_fpes(false);
  constexpr size_t num_points = 1;
  // Non-round adiabatic index so mistaken powers (rho vs rho^2, ...) cannot
  // hide behind unit values.  The spatial metric is kept flat: the eigen-
  ...
  std::ofstream dump;
  if (output) {
    dump.open("degeneracy_parameter_sweep.tsv",
              std::ios::out | std::ios::trunc);
...
void test_flux_jacobian_precision(const bool output) {
  ...
  for (const auto& st : states) {
    ...
    // using the LITERAL (Gamma-1) and the same double-precision h (cast to
    // quad) that is passed to the double function.
    ...
    for (size_t ib = 0; ib < n_bn; ++ib) {
      ...
      const double bn_frac = std::pow(
          10.0, std::log10(0.5) + (std::log10(1.0e-9) - std::log10(0.5)) *
                                      static_cast<double>(ib) /
                                      static_cast<double>(n_bn - 1));
      const double bx = bn_frac * b_mag;
      const double bt = std::sqrt(std::max(0.0, square(b_mag) - square(bx)));
      ...
      // not a mere round-off residue of an analytically-zero entry.  We treat
      // an entry as significant when its magnitude exceeds 1e-8 times the
      // largest entry of the matrix (matrix inf-scale).  Below that floor a
      // nominally zero entry can show up as ~1e-16 in one precision and a
      // different ~1e-16 in the other, giving a meaningless relative error of
      // order unity; those entries carry no accuracy loss because their
      // absolute value is negligible against the matrix scale.  The reported
      // max_rel_err therefore measures genuine loss of significant digits in
      // the meaningful entries, which is exactly what caps the eigensolver.
      double matrix_scale = 0.0;
      for (size_t m = 0; m < 9; ++m) {
        for (size_t n = 0; n < 9; ++n) {
          matrix_scale =
              std::max(matrix_scale, std::abs(jacobian.get(m, n)[0]));
          matrix_scale = std::max(matrix_scale,
                                  std::abs(static_cast<double>(q_jac[m][n])));
        }
      }
...
// speeds must approach the hydro acoustic speeds v_n +- c_s
// (slow/Alfven/entropy
...
void test_hydro_mhd_comparison(const bool output) {
  ...
  // [S_x,S_y,S_z,B^x,B^y,B^z,D,tau,phi]; hydro order is
  // [D,S_x,S_y,S_z,tau,Y_e].
  ...
  // Parameter grid: thermal pressure (cs^2) x field orientation bn_frac =
  // B_n/|B| (proximity to the field degeneracy at a given |B|).  W, phi, vn
  // fixed.
  const double W = 1.4;
  ...
  if (output) {
    ...
    dump << std::setprecision(16);
    dump
        << "cfg\tpressure\tbn_frac\tW\tsigma\tb_norm\tmatrix_fluid_diff\t"
    ...
    for (const double bn_frac : bn_fracs) {
      ...
      for (size_t is = 0; is < n_sig; ++is) {
        const double sigma = std::pow(
            10.0, 1.0 + (std::log10(1.0e-9) - 1.0) * static_cast<double>(is) /
        ...
        try {
          ...
          for (size_t a = 0; a < 5; ++a) {
            for (size_t b = 0; b < 5; ++b) {
              ...
              e = std::max(e, std::abs(val - jac_mhd.get(mhd_fluid[a],
                                                         mhd_fluid[b])[0]));
            }
          }
          err_mhd_recon = e;
...
// biorthonormalization).  For DISTINCT eigenvalues, left/right eigenvectors of
// a matrix are automatically biorthogonal (L_i.R_j = 0, i != j), so a scalar
...
//   raw_offdiag = max_{i!=j} |L_i.R_j|            (biorthogonality of geev
//   output) raw_mindiag = min_i |L_i.R_i|                 (how far the diagonal
//   is from 1) rescaled_id = || sum_i R_i (L_i/(L_i.R_i))^T - I ||   (does a
//   scalar rescale fix it)
...
void test_numeric_biorthogonality(const bool output) {
  ...
  spatial_velocity.get(2) = 0.0;
  const double b_mag =
      std::sqrt(sigma * get(rest_mass_density)[0] * get(specific_enthalpy)[0]);
  ...
  // eigenvectors must reconstruct the identity -- i.e. the numeric
  // decomposition needs no inversion there, only per-wave renormalization.
  CHECK(well_sep_rescaled_id < 1.0e-9);

  // Contrast: the HYDRO system has an EXACT 4-fold degeneracy at v_n (the
  // contact modes), so geev returns an arbitrary basis for that eigenspace and
  // its left / right bases are NOT mutually biorthogonal -- a scalar rescale
  // then does NOT recover the identity.  This is the genuine "degenerate block"
  // problem (and the likely cause of the earlier HydroYe numeric-Marquina
  // failure).
  {
    ...
    for (size_t a = 0; a < 6; ++a) {
      for (size_t b = 0; b < 6; ++b) {
        ...
        h_rescaled_id =
            std::max(h_rescaled_id, std::abs(v - (a == b ? 1.0 : 0.0)));
      }
    }
    if (output) {
    ...
    // The exact degenerate block breaks biorthogonality: a scalar rescale is
    // not enough (the block needs a proper projector, e.g. the complementary
    // one).
...
// |A| = sum_i |lambda_i| R_i (L_i/(L_i.R_i))^T (the decomposition-dependent
// part of the numerical flux).  Over the parameter-space corners, sweep B_n ->
// 0 and compare, against the flux Jacobian A = flux_jacobian_mhd:
//   err_signed_analytic = || sum_i lambda_i R_i^an  L_i^an  - A ||   (grows
//   near deg.) err_signed_numeric  = || sum_i lambda_i R_i^num L_i^num - A ||
//   (~round-off) diss_diff           = || |A|_analytic - |A|_numeric ||
//   (analytic flux error)
...
// correction -- not the flux Jacobian.  For each parameter-space corner we
// build a physical interface (an interior state and a slightly perturbed
// exterior state), compute the real conserved variables and normal fluxes with
// ConservativeFromPrimitive / Fluxes, and then apply the exact Marquina
// per-wave flux formula (copied from Marquina::dg_boundary_terms) with EACH
// decomposition. The numeric decomposition is the reference; we report the
// relative difference of the analytic boundary correction from it, swept over
// B_n -> 0.  Flat metric (lapse 1, shift 0, sqrt(gamma) 1), interior normal +x
// / exterior normal -x. Dump guarded by SPECTRE_NUMEIG_DUMP.
void test_numeric_vs_analytic_flux(const bool output) {
  ...
  // [S_x,S_y,S_z,B_x,B_y,B_z,D,tau,phi]) and normal flux F.n for a given
  // normal.
  struct State {
  ...
  const auto make_state =
      [&](double density, double pressure_val, double W_lor, double vn_frac,
          double bn_frac, double b_mag, double phi_val, double phi_ang,
          const tnsr::i<DataVector, 3, Frame::Inertial>& nrm) {
        State s;
        s.rho = Scalar<DataVector>{DataVector(num_points, density)};
        const Scalar<DataVector> pressure{DataVector(num_points, pressure_val)};
        s.eps = eos_2d.specific_internal_energy_from_density_and_pressure(
            s.rho, pressure);
        ...
        grmhd::ValenciaDivClean::ConservativeFromPrimitive::apply(
            ...
            make_not_null(&tilde_b), make_not_null(&tilde_phi), s.rho, s.ye,
            s.eps, pressure, s.velocity, s.W, s.bfield, sqrt_det,
            spatial_metric, s.phi);
        tnsr::I<DataVector, 3, Frame::Inertial> f_d{num_points},
            f_ye{num_points}, f_tau{num_points}, f_phi{num_points};
        ...
        const auto ndot_vec =
            [&](const tnsr::I<DataVector, 3, Frame::Inertial>& fl) {
              double v = 0.0;
              for (size_t i = 0; i < 3; ++i) {
        ...
        s.fn = {ndot(f_s, 0),  ndot(f_s, 1),    ndot(f_s, 2),
                ndot(f_b, 0),  ndot(f_b, 1),    ndot(f_b, 2),
                ndot_vec(f_d), ndot_vec(f_tau), ndot_vec(f_phi)};
        return s;
      };
  ...
  for (size_t ic = 0; ic < cfgs.size(); ++ic) {
    ...
    for (size_t ib = 0; ib < n_bn; ++ib) {
      ...
      const State se =
          make_state(density * 1.05, cf.pressure * 1.04, W * 1.01, 0.3, bn_frac,
                     b_mag * 1.03, 0.025, phi_ang, normal_ext);
      ...
      const bool numeric_ok =
          di_nu.ok and de_nu.ok and biorth_ok(di_nu) and biorth_ok(de_nu);
...
// Wave-structure dumper: read an exact Riemann profile (x, rho, p_tot, vx, vy,
// vz, By, Bz) plus the constant B_x and adiabatic index, reconstruct the
// primitive state per point (p_gas = p_tot - b^2/2), and write the nine MHD
// characteristic speeds vs x using SpECTRE's own characteristic_speeds_mhd. The
