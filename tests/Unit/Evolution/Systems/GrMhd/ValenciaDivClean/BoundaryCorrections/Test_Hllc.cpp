...
SPECTRE_TEST_CASE("Unit.GrMhd.ValenciaDivClean.BoundaryCorrections.Hllc",
                  "[Unit][GrMhd]") {
  PUPable_reg(grmhd::ValenciaDivClean::BoundaryCorrections::Hllc);
  ...
  using Hllc = grmhd::ValenciaDivClean::BoundaryCorrections::Hllc;

  const Hllc hllc_boundary_correction{1.0e-30, 1.0e-8};
  ...
  helpers::test_boundary_correction_with_python<system,
      ...
      make_not_null(&gen), "Hllc", "dg_package_data", "dg_boundary_terms",
  ...
  const auto factory_hllc =
      TestHelpers::test_factory_creation<evolution::BoundaryCorrection, Hllc>(
          "Hllc:\n"
  ...
  helpers::test_boundary_correction_with_python<system,
                                                tmpl::list<ConvertPolytropic>>(
      make_not_null(&gen), "Hllc", "dg_package_data", "dg_boundary_terms",
      dynamic_cast<const Hllc&>(*factory_hllc), mesh, volume_data, ranges);

  // Equality operators.
  CHECK_FALSE(Hllc{1.0e-30, 1.0e-8} != Hllc{1.0e-30, 1.0e-8});

  CHECK(Hllc{1.0e-30, 1.0e-8} != Hllc{2.0e-30, 1.0e-8});

  CHECK(Hllc{1.0e-30, 1.0e-8} != Hllc{1.0e-30, 2.0e-8});
}
}  // namespace
