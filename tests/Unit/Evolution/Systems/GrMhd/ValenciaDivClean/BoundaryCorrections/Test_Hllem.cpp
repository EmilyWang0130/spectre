...
InterfaceState make_state(const double rest_mass_density, const double pressure,
                          ...
                          const double adiabatic_index,
                          const size_t num_points) {
  InterfaceState state = make_state_at_rest(
      rest_mass_density, pressure, magnetic_field, adiabatic_index, num_points);
...
void test_vanishing_tangential_field_stays_finite() {
  ...
  for (const double tangential_b :
    ...
    const std::array<double, 3> magnetic_field{{1.0, tangential_b, 0.0}};
    const auto state_int = make_state_at_rest(1.0, 1000.0, magnetic_field,
                                              adiabatic_index, num_points);
    const auto state_ext = make_state_at_rest(0.1, 1.0, magnetic_field,
                                              adiabatic_index, num_points);
...
void test_stationary_contact_is_exact(const double density_ratio,
  ...
  for (const auto waves :
    ...
    const DataVector lambda_max = max(0.0, get(interior.largest_outgoing),
                                      -get(exterior.largest_ingoing));
    const DataVector lambda_min = min(0.0, get(interior.largest_ingoing),
                                      -get(exterior.largest_outgoing));
    ...
    const double residual_tilde_tau =
        fabs(get(correction_tilde_tau)[0]) / scale;
    CAPTURE(waves);
    ...
    CHECK(scale >
          0.1 * fabs(get(exterior.tilde_d)[0] - get(interior.tilde_d)[0]));
    if (require_exact) {
